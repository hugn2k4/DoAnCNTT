...
<section class="section-content" style="margin: 100px 0;">
  <div class="card mx-auto" style="max-width: 380px">
    <div class="card-body">
      ...
      <c:if test="${empty requestScope.showOtpForm}">
      ...
      <c:if test="${not empty requestScope.showOtpForm}">
        <h4 class="card-title mb-4">Xác nhận mã OTP</h4>
        <form action="${pageContext.request.contextPath}/signup" method="post">
          <input type="hidden" name="action" value="verifyOtp">
          <div class="mb-3">
            <label for="inputOtp" class="form-label">Nhập mã OTP</label>
            <input type="text"
                   class="form-control ${not empty requestScope.otpError ? 'is-invalid' : ''}"
                   id="inputOtp"
                   name="otp"
                   required>
            <c:if test="${not empty requestScope.otpError}">
              <div class="invalid-feedback">${requestScope.otpError}</div>
            </c:if>
          </div>
